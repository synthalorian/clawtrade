...
pub async fn market_trends(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let mut trends: std::collections::HashMap<String, (Vec<i64>, i64)> =
    ...
    let result: Vec<MarketTrend> = trends
        .into_iter()
        .map(|(service_type, (prices, velocity))| {
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "trends": result })),
    )
}

/// GET /api/v1/pricing/recommendations — per-service pricing advice
pub async fn pricing_recommendations(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let services = match Service::list(&state.pool).await {
        Ok(s) => s,
        Err(e) => {
            return (
    ...
    let mut type_prices: std::collections::HashMap<String, Vec<i64>> =
        std::collections::HashMap::new();
    ...
    for svc in &services {
        let avg_price = type_prices
            .get(&svc.service_type)
            .map(|p| {
                if !p.is_empty() {
                    p.iter().sum::<i64>() / p.len() as i64
                } else {
                    svc.price_cents
                }
            })
        ...
        let same_type_count = type_prices
            .get(&svc.service_type)
            .map(|p| p.len() as i64)
            .unwrap_or(0);
        let sales_24h = type_sales.get(&svc.id).copied().unwrap_or(0);
        let sales_1h = transactions
            .iter()
            .filter(|t| {
                t.service_id == svc.id
                    && t.created_at > hour_ago
                    && (t.status == "paid" || t.status == "escrow" || t.status == "released")
            })
            .count() as i64;
        ...
        let (recommended, reason) = if same_type_count >= 3 {
            ...
            (
                new_price,
                format!(
                    "3+ competitors. Undercut avg by 10% (avg: ${}.{}",
                    avg_price / 100,
                    avg_price % 100
                ),
            )
            ...
            (
                new_price,
                format!(
                    "Only service of type. Premium 20% over avg (avg: ${}.{}",
                    avg_price / 100,
                    avg_price % 100
                ),
            )
            ...
            (
                new_price.max(50),
                "No sales in 24h. Drop 10% to stimulate demand.".to_string(),
            )
        } else if sales_1h >= 5 {
            ...
            (
                new_price,
                format!("High demand ({} sales/hr). Raise 15%.", sales_1h),
            )
        } else {
            (
                svc.price_cents,
                "Market price is optimal. No change recommended.".to_string(),
            )
        };
