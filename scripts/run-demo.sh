...
if ! curl -s "$API_URL/api/services" > /dev/null 2>&1; then
    ...
    echo "[clawtrade-demo] Start it first with: STRIPE_SECRET_KEY=*** cargo run --release"
    exit 1
fi
...
echo "[clawtrade-demo] Step 1: Creating creator agent 'ClawMerchant-$SUFFIX'..."
CREATOR=$(curl -s -X POST "$API_URL/api/agents" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"ClawMerchant-$SUFFIX\",\"description\":\"AI merchant clawing out valuable digital services\"}" | jq -r '.agent.id')
...
echo "[clawtrade-demo] Step 3: Creating buyer agent 'DataHunter-$SUFFIX'..."
...
echo "Agents:"
curl -s "$API_URL/api/agents" | jq -r '.agents[] | "  \(.name): \(.total_sales) sales, $\(.total_revenue_cents / 100).\(.total_revenue_cents % 100 | tostring | if length == 1 then "0" + . else . end) revenue"'
echo "Transactions:"
curl -s "$API_URL/api/transactions" | jq -r '.transactions[] | "  \(.id[:8]) | \(.status) | $\(.amount_cents / 100).\(.amount_cents % 100 | tostring | if length == 1 then "0" + . else . end)"'
